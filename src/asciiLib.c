...
asciiBitmap asciiCreateFilledBitmapEx (asciiPoint size,asciiChar ch) {
	...
	if (bitmap.address)
		memset(bitmap.address,ch.character,memlen);
...
void asciiDrawRectangle (asciiEngine* e,asciiRect rect) {
	if (e)
		asciiDrawRectangleColored(e,rect,e->clearChar.backColor,e->clearChar.foreColor);
}
void asciiDrawRectangleColored (asciiEngine* e,asciiRect rect,asciiColor bg,asciiColor fg) {
	if (e && rect.size.x>2 && rect.size.y>2 && bg<ASCII_COLOR_COUNT && fg<ASCII_COLOR_COUNT) {
		asciiFillRect(e,asciiChar('-',bg,fg),asciiRect(rect.offset.x+1,rect.offset.y,rect.size.x-2,1));
		asciiFillRect(e,asciiChar('-',bg,fg),asciiRect(rect.offset.x+1,rect.offset.y+rect.size.y-1,rect.size.x-2,1));
		asciiFillRect(e,asciiChar('|',bg,fg),asciiRect(rect.offset.x,rect.offset.y+1,1,rect.size.y-2));
		asciiFillRect(e,asciiChar('|',bg,fg),asciiRect(rect.offset.x+rect.size.x-1,rect.offset.y+1,1,rect.size.y-2));
		asciiDrawChar(e,asciiChar('+',bg,fg),asciiPoint(rect.offset.x,rect.offset.y));
		asciiDrawChar(e,asciiChar('+',bg,fg),asciiPoint(rect.offset.x+rect.size.x-1,rect.offset.y));
		asciiDrawChar(e,asciiChar('+',bg,fg),asciiPoint(rect.offset.x,rect.offset.y+rect.size.y-1));
		asciiDrawChar(e,asciiChar('+',bg,fg),asciiPoint(rect.offset.x+rect.size.x-1,rect.offset.y+rect.size.y-1));
	}
}
void asciiDrawFilledRectangle (asciiEngine* e,asciiRect rect) {
	if (e)
		asciiDrawFilledRectangleColored(e,rect,e->clearChar.backColor,e->clearChar.foreColor);
}
void asciiDrawFilledRectangleColored (asciiEngine* e,asciiRect rect,asciiColor bg,asciiColor fg) {
	if (e && rect.size.x>2 && rect.size.y>2 && bg<ASCII_COLOR_COUNT && fg<ASCII_COLOR_COUNT) {
		asciiDrawRectangleColored(e,rect,bg,fg);
		asciiFillRect(e,asciiChar(' ',bg,fg),asciiRect(rect.offset.x+1,rect.offset.y+1,rect.size.x-2,rect.size.y-2));
	}
}
